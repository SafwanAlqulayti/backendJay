...
export class OrderService {
  ...
  async create(createOrderDto: CreateOrderDto): Promise<Order> {
    const user = await this._authService.findOne(createOrderDto.userId);
    const restaurant = await this._restaurantService.findOne({
      id: createOrderDto.restaurantId,
    }); //check restauran staus befoure order

    if (!user) {
      throw new BadRequestException();
    }
    const meals = await this._mealService.getMealsForOrder(
      createOrderDto.mealsIds,
    );
  ...
  async historyOrder(histroyOrderDto: HistroyOrderDto) {
    const orders = await this._orderRepo.find({
      where: { user: histroyOrderDto.userId, status: OrderStatus.COMPLATED },
  ...
  async orderDetail(orderDetailDto: OrderDetailDto) {
    const order = await this._orderRepo.findOne({
      where: { id: orderDetailDto.orderId },
      relations: ['meals'],
    });
  ...
  async restaurantOrders(restataurantOrdersDto: RestataurantOrdersDto) {
    const orders = await this._orderRepo.find({
      where: { restaurant: restataurantOrdersDto.restaurantId },
  ...
  async update(updateOrderDto: UpdateOrderDto, user) {
    ...
    const order = await this._orderRepo.findOne(updateOrderDto.orderId);
    if (!order) throw new BadRequestException('There is no order with this id');

    await this._orderRepo
      .createQueryBuilder()
      .update(Order)
      .set({
    ...
    return {
      message: `The order status has been changed to ${updateOrderDto.status}`,
    };
  }
}
