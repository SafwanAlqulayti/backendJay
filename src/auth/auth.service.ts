...
export class AuthService {
  ...
  async signIn(CreateAdminDto: SignInDto): Promise<{ accessToken: string }> {
    const User = await this.UserRepository.signIn(CreateAdminDto);
    //    if (User.IsActive == false) {

    if (!User) {
      throw new BadRequestException(
        await this._i18nService.translate('businessError.notRegister'),
      );
    }
    if (User.email === null) {
  ...
  async sendOTP(sendOTP: SendOTP) {
    const phoneNumber = phone(sendOTP.phoneNumber, 'SAU')[0];
    const user = await this.UserRepository.findOne({
